...
\subsubsection{物理可达状态集}
\label{sec:domain}

\textbf{关键约束}：由初始条件 $x(0) = x_0 = 6314 < K_{\text{eff}}$，在 logistic 动力学
$\dot{x} = f(x)$ 作用下，$f(x) > 0$ 对任意 $x \in [x_0, K_{\text{eff}})$ 成立，
故 $x(t)$ 从 $x_0$ 出发后单调递增，且以 $K_{\text{eff}}$ 为上界。定义物理可达状态集：
\begin{equation}
...
\textbf{在物理可达状态集内，系统只有一个平衡点}：$x^* = K_{\text{eff}} = 8142.22$ 吨/天。
数学上的 $x^*_1 = 0$ 虽然在方程中作为一个不动点存在，但它位于 $\mathcal{D}$ 之外，
\textbf{不具有物理可达性}——回收量从 $x_0$ 出发后单调递增，永远不会降至 $x_0$ 以下，
更不可能达到零。这一观察对后续稳定性讨论至关重要。
...
在物理可达状态集 $\mathcal{D} = [x_0, K_{\text{eff}}]$ 内，$x^*_2 = K_{\text{eff}}$ 是唯一的平衡点。
...
在物理可达状态集 $[x_0, K_{\text{eff}}) \cup (K_{\text{eff}}, +\infty)$ 内分析 $f(x)$ 的符号：
\begin{align}
...
\begin{itemize}
  ...
  \item 物理可达状态集 $\mathcal{D} = [x_0, K_{\text{eff}}]$ 是 $\mathbb{R}$ 的闭子集
\end{itemize}
...
\begin{itemize}
  ...
  \item 物理可达状态集 $x \in [x_0, K_{\text{eff}}]$ 也不支持负值的不动点
\end{itemize}
...
更实际的评估——引入基于观测窗口的性能阈值。
取 $r_{\min} \triangleq \ln(100)/(2.5T)$，其中 $T = 365$ 天为数据观测周期，
$2.5T$ 为最大可接受的 $99\%$ 收敛时间（保守上界 $t_{99\%} \leq \ln(100)/r$），
得 $r_{\min} = \ln(100)/912.5 \approx 0.00505$，取整为 $0.005$。
计算使 $r$ 降低至此临界值所需的 $\beta$ 增量：
\begin{equation}
...
在物理可达状态集 $\mathcal{D} = [x_0, K_{\text{eff}}]$ 内，系统有且仅有一个不动点 $x^* = K_{\text{eff}}$。
...
在整个有效参数空间内：
\begin{itemize}
  \item 不动点的数量不变（数学上 2 个，物理可达状态集内 1 个）
