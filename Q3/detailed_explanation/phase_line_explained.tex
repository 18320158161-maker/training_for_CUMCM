...
$x$ 的物理可达状态集为 $\mathcal{D} = [x_0, K_{\text{eff}}]$（由 $x_0 = 6314 < K_{\text{eff}}$ 及 $f(x) > 0$（$\forall x \in [x_0, K_{\text{eff}})$）保证 $x(t)$ 单调递增且以 $K_{\text{eff}}$ 为上界；$x > K_{\text{eff}}$ 区域数学上存在但物理轨迹不可达）。
...
（数学上 $x = 0$ 也是根且不稳定——这恰好解释了为什么回收体系不会自发``死掉''，但 $x = 0$ 在物理可达状态集 $\mathcal{D} = [x_0, K_{\text{eff}}]$ 之外，不是系统的有效平衡点。）
...
错误。物理可达状态集为 $\mathcal{D} = [x_0, K_{\text{eff}}]$（从初值出发的轨迹单调递增且以 $K_{\text{eff}}$ 为上界），
