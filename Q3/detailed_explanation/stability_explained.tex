...
代数方程 $f(x) = 0$ 有两个数学解，但由 $x_0 = 6314 < K_{\text{eff}}$ 及 $f(x) > 0$（$\forall x \in [x_0, K_{\text{eff}})$），从 $x_0$ 出发的轨迹单调递增且以 $K_{\text{eff}}$ 为上界，物理可达状态集为 $\mathcal{D} = [x_0, K_{\text{eff}}]$，$x_1^* = 0$ 远在此集之外。\\
\textbf{物理上只存在一个平衡点：}
...
用相线方法交叉验证线性化的结论。物理可达状态集为 $\mathcal{D} = [x_0, K_{\text{eff}}]$（从 $x_0 < K_{\text{eff}}$ 出发，$f(x) > 0$ 保证 $x$ 单调递增且以 $K_{\text{eff}}$ 为上界），平衡点 $x^* = K_{\text{eff}}$ 将可达集切为区间 I 及以外区域：
...
\textbf{$r_{\min}$ 如何确定？} 我们需要一条有物理依据的定量准则，而非主观取数。

Logistic 系统从初值恢复至 $99\%$ 饱和水平所需时间满足保守上界：
\[
t_{99\%} \leq -\frac{\ln(0.01)}{r} = \frac{\ln(100)}{r}
\]
（此上界忽略初值因子 $K_{\text{eff}}/x_0-1 \approx 0.29$ 的加速效应——
因子小于 1 时实际收敛更快，因此 $-\ln(0.01)/r$ 给出的是最不利估计。）

现要求系统在\textbf{不超过 $2.5$ 个观测周期内基本完成收敛}：
观测周期 $T = 365$ 天（表(b) 数据时间跨度），
$2.5T = 2.5 \times 365 = 912.5$ 天 $\approx 2.5$ 年。
这一时长在市政规划尺度上是合理的——任何回收体系的运营策略调整
不应等待 $3$ 年以上才判断是否生效。

由 $t_{99\%} \leq 2.5T$ 得：
\[
\frac{\ln(100)}{r} \leq 912.5
\;\Rightarrow\;
r \geq \frac{\ln(100)}{912.5} = \frac{4.60517}{912.5} \approx 0.00505
\]

取整至合理精度，定义：
\[
\boxed{r_{\min} = 0.005\ \text{天}^{-1}}
\]
代入 $\beta_{\max} = \gamma/r_{\min} - 1$：
\[
...
\begin{enumerate}[label=\textbf{步骤 \arabic*}.]
  ...
  \item \textbf{找平衡点}：令 $f(x) = r x (1 - x/K_{\text{eff}}) = 0$。物理可达状态集为 $\mathcal{D} = [x_0, K_{\text{eff}}]$，
        只有 $x^* = K_{\text{eff}}$ 在可达集内。（数学上另有一根 $x=0$，远在可达集外。）
