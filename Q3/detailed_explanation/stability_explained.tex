...
\boxed{x_1^* = 0, \qquad x_2^* = K_{\text{eff}} = K(1+\alpha)}
\end{equation}

\textbf{物理含义}：
\begin{itemize}
  \item $x_1^* = 0$：回收量为零——体系完全瘫痪的状态。
  \item $x_2^* = K_{\text{eff}}$：回收量达到有效饱和容量的状态。
\end{itemize}
...
\subsection{评估每个平衡点的 $\lambda = f'(x^*)$}

\subsubsection*{平衡点 $x_1^* = 0$}
...
\textbf{结论}：$\lambda_1 > 0$ $\Rightarrow$ $x_1^* = 0$ 是\textbf{不稳定的}。

\textbf{物理含义}：只要稍微有一点点回收活动（$x(0) > 0$，哪怕很小），系统就会自动远离零状态，开始增长。这很好——回收体系不会自己``死掉''。

\subsubsection*{平衡点 $x_2^* = K_{\text{eff}}$}
...
\textbf{结论}：$\lambda_2 < 0$ $\Rightarrow$ $x_2^* = K_{\text{eff}}$ 是\textbf{渐近稳定的}。

\textbf{物理含义}：如果回收量因为某种原因偏离了有效容量（比如某个月因节假日波动偏高或偏低），系统会自动调节、回到 $K_{\text{eff}}$。这是一个``自愈''机制。

\subsection*{数值代入}
\[
\lambda_1 = r = 0.008485 > 0 \quad \text{（不稳定）}, \qquad
\lambda_2 = -r = -0.008485 < 0 \quad \text{（渐近稳定）}
\]
...
\midrule
$x_1^*$（零回收）  & $0$            & $0$      & $\lambda_1 = r = 0.008485 > 0$  & \textbf{不稳定} \\
$x_2^*$（可持续） & $K(1+\alpha)$  & $8142.22$ & $\lambda_2 = -r = -0.008485 < 0$ & \textbf{渐近稳定} \\
\bottomrule
\end{tabular}
\end{center}
...
用相线方法交叉验证线性化的结论。注意：我们只关心 $x \geq 0$（回收量不能为负），
$f(x) = r \cdot x \cdot (1 - x/K_{\text{eff}})$ 是三个因子的乘积，$r > 0$ 恒成立。

\subsection*{两个平衡点把正半轴切成两个区间}
...
\begin{itemize}
  \item \textbf{$x_2^* = K_{\text{eff}}$}：左边箭头向右 $\rightarrow$、右边箭头向左 $\leftarrow$，
        \textbf{两边都指向它} $\Rightarrow$ \textbf{渐近稳定}。
        不管初始回收量是偏低还是偏高，系统都自动回到 $K_{\text{eff}}$。
  \item \textbf{$x_1^* = 0$}：右边的箭头向右 $\rightarrow$（背离它），
        \textbf{箭头背离它} $\Rightarrow$ \textbf{不稳定}。
        只要 $x$ 稍微大于零，系统就自动远离零。
\end{itemize}

\textbf{物理直觉}：
\begin{itemize}
  \item 回收量在 $(0, K_{\text{eff}})$ 之间 → 还有剩余空间，系统自动增长
  \item 回收量超过 $K_{\text{eff}}$ → 超出承载能力，系统自动回落
  \item 不管从哪出发（只要 $x(0) > 0$），最终都稳定在 $K_{\text{eff}}$
\end{itemize}
...
\fbox{%
\begin{minipage}{0.88\textwidth}
\textbf{对任意正初值 $x(0) > 0$}（即初始时至少有一点点回收活动），\\
\textbf{对任意 $\alpha \geq 0$, $\beta \geq 0$}（即促进和抑制都是正向的），\\
系统\textbf{恒收敛}到 $x_2^* = K_{\text{eff}} > 0$——一个正的有效容量。
\end{minipage}}
\end{center}
...
\;\Rightarrow\; \lambda_2 = -r < 0 \text{ 恒成立}
\]

\textbf{这意味着}：在整个有效的参数空间内（$\alpha \geq 0, \beta \geq 0$），稳定平衡点 $x_2^*$ 永远是稳定的，稳定性的符号永远不会翻转。系统不会因为参数变化而发生质的改变（比如突然从一个稳态跳到另一个不稳定的状态，或者平衡点消失）。
...
\]
当 $\beta < -1$ 时：$1+\beta < 0$，所以 $r < 0$，这会导致 $\lambda_2 = -r > 0$——稳定平衡点变成了不稳定的！
...
因为稳定性由 $\lambda_2 = -r = -\gamma/(1+\beta)$ 的符号决定。
而 $\text{sign}(\lambda_2) = \text{sign}(-\gamma/(1+\beta))$。
...
\begin{enumerate}[label=\textbf{步骤 \arabic*}.]
  ...
  \item \textbf{找平衡点}：令 $f(x) = r x (1 - x/K_{\text{eff}}) = 0$，得
        \[
        x_1^* = 0, \quad x_2^* = K_{\text{eff}}
        \]

  \item \textbf{线性化}：计算 $f'(x) = r(1 - 2x/K_{\text{eff}})$，在每个平衡点评估：
        \[
